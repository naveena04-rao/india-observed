...
const patterns = [
  ...
  { name: "Non-empty Supabase service key assignment", regex: /SUPABASE_SERVICE_ROLE_KEY[ \t]*=[ \t]*[^\r\n#][^\r\n]{15,}/g }
];
...
for (const file of tracked) {
  ...
  for (const pattern of patterns) {
    ...
    for (const match of matches) findings.push({ file, type: pattern.name, preview: match.slice(0, 24) });
  }
}
