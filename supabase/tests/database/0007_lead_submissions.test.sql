...
select is(has_function_privilege('anon', 'public.submit_lead(text,text,text,date,text,text[],text,text,text,text)', 'execute'), false, 'anonymous RPC execution is denied');
select is(has_function_privilege('authenticated', 'public.submit_lead(text,text,text,date,text,text[],text,text,text,text)', 'execute'), false, 'authenticated RPC execution is denied');
select is(has_function_privilege('service_role', 'public.submit_lead(text,text,text,date,text,text[],text,text,text,text)', 'execute'), true, 'service role can execute insertion boundary');

set local role service_role;
select lives_ok(
  $$select public.submit_lead('Public meeting announced', repeat('Factual description. ', 4), 'New Delhi', date '2026-07-31', 'exact', array['https://example.org/report'], null, 'editor@example.org', '+44 20 7946 0958', repeat('a', 64))$$,
...
select throws_ok(
  $$select public.submit_lead('Public meeting announced', repeat('Factual description. ', 4), 'New Delhi', date '2026-07-31', 'exact', '{}', null, 'editor@example.org', null, repeat('a', 64))$$,
  '23505',
  null,
