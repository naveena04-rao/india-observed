...
select is((select count(*)::integer from public.scan_sources where enabled and scan_method = 'rss'), 2, 'both daily sources use RSS');
select is((select count(*)::integer from public.scan_sources where enabled and daily_request_limit <= 2), 2, 'both sources have bounded retry-aware request limits');
select is((select count(*)::integer from public.scan_sources where enabled and connector_config ->> 'fullArticleFetching' = 'false'), 2, 'full article fetching is disabled');
select is((select count(*)::integer from public.scan_sources where enabled and connector_config ->> 'mediaFetching' = 'false'), 2, 'media fetching is disabled');
