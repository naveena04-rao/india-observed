...
select is((select count(*)::integer from public.scan_sources where enabled and scan_frequency = 'daily'), 30, 'thirty full-scale daily sources are enabled');
select is((select count(*)::integer from public.scan_sources where enabled and scan_method = 'rss'), 30, 'all focused sources use RSS');
...
select is((select count(*)::integer from public.scan_sources where enabled and connector_config ->> 'timeWindowHours' = '96'), 30, 'all sources use a 96-hour window');
select is((select count(*)::integer from public.scan_sources where enabled and connector_config ->> 'fullArticleFetching' = 'false'), 30, 'unbounded full article fetching remains disabled');
select is((select count(*)::integer from public.scan_sources where enabled and connector_config ->> 'mediaFetching' = 'false'), 30, 'media fetching remains disabled');
